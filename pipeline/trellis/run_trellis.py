...
def patch_lazy_weights() -> None:
    ...
    def release(model) -> None:
        ...
        # Модели разного размера (0.14, 0.88 и 2.44 ГБ) сменяют друг друга по
        # шесть раз за прогон, поэтому кэш аллокатора чистится: иначе куски
        # разной длины со временем дробят свободную память.
